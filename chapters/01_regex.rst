...
.. index:: Introduction to regular expressions
...
Regular expressions are a means to describe a text pattern, [#1]_ so that you can look for 
...
.. rubric:: Regular Expression Vocabulary
...
.. list-table::
   ...
   * - ``?``
     - Match the previous thing zero or one times. In other words, it makes it optional. It also makes the ``*`` and ``+`` characters non-greedy. See :ref:`Greedy`.
   * - ``{n,m}``
     - Indicates that the previous thing should match at least n, and not more than m times. For example, ``a{2,7}`` matches at least 2, and not more than 7, occurrences of the letter a
   * - ``( )``
...
.. list-table::
   ...
   * - Character
     - Meaning
   * - \d
     - Match any character in the range 0 - 9
   * - \D
     - Match any character NOT in the range 0 - 9
   * - \s
     - Match any whitespace characters (space, tab etc.).
   * - \S
     - Match any character NOT whitespace (space, tab).
   * - \w
     - Match any character in the range 0 - 9, A - Z and a - z
   * - \W
     - Match any character NOT the range 0 - 9, A - Z and a - z
   * - \b
     - Word boundary. Match any character(s) at the beginning (``\babc``) and/or end (``abc\b``) of a word, thus ``\bcow\b`` will match cow but not cows, but ``\bcow`` will match cows.
   * - \B
     - Not a word boundary. Match any character(s) NOT at the beginning(``\Babc``) and/or end (``cow\B``) of a word, thus ``\Bcow\B`` will match scows but not cows, but ``cow\B`` will match coward.
   * - \t
     - Match a tab character
   * - \n
     - Match a newline character
   * - \x
     - Matches a character with a particular hex code. For example, ``\x5A`` would match a Z, which has a hex code of 5A.
...
included in this basic vocabulary. Consider the examples in the ``anchor examples table``

.. index:: Anchor examples


.. rubric:: Anchor examples
...
expression. The ``table of greedy examples`` shows the results of these
patterns.
...
This matches any path not starting with ``/images``. We'll see more of this kind of pattern match especially in the chapter :ref:`Chapter_proxy`.
...
See the section *Repetition quantifiers* for the various syntaxes you
